...
const UploadImageStatusModal = () => {
  ...
  const handleClose = () => {
    setUrls([]);
    setShowUploadStatusImagesModal(false);
  ...
  const handleUpload = async () => {
    try {
      await setUploadStatusImages(uploadStatusImages.concat(urls));
      handleClose();
    } catch (error) {
      console.log(error.message);
    }
  ...
  return (
    <Modal
      show={showUploadStatusImagesModal}
      onHide={handleClose}
      keyboard={false}
    >
      <Modal.Header>
        <h4>Status Images Upload</h4>
...
export default UploadImageStatusModal;
