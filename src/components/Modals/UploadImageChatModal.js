...
const UploadImageChatModal = () => {
  ...
  const handleClose = () => {
    setUrls([]);
    setShowUploadMessageImagesModal(false);
    /* urls.forEach((url) => {
  ...
  const handleUploadFiles = (path, file) => {
    ...
    if (file) {
      const imageRef = ref(
        storage,
        `${userDoc?.email}/${path}/messages/${file.name}-${uuidv1()}`
      );
      const metadata = {
  ...
  const handleUpload = async () => {
    try {
      await setUploadMessageImgages(uploadMessageImgages.concat(urls));
      handleClose();
  ...
  return (
    <Modal
      show={showUploadMessageImagesModal}
      onHide={handleClose}
      keyboard={false}
    >
      <Modal.Header>
...
export default UploadImageChatModal;
